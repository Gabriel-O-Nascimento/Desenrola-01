...
import { normalizeProfessional, normalizeSection } from "../../utils/normalize";
...
function SolicitarServico() {
  ...
  useEffect(() => {
    api.getHome()
      .then((response) => {
        const sections = response.map(normalizeSection);
        setServices(sections.flatMap((section) => section.items.map((item) => ({
          ...item,
  ...
  }, [id, professional, professionalId, services]);
  ...
  function validateForm() {
    ...
    if (!formData.address.trim()) {
      newErrors.address = "Informe o endereco do servico.";
    }
    ...
    if (!formData.date) {
      ...
      newErrors.date = "Informe uma data valida.";
    } else if (isDateBeforeToday(formData.date)) {
      newErrors.date = "A data nao pode ser anterior a data atual.";
    }

    if (!formData.time) {
      newErrors.time = "Informe o horario desejado.";
    } else if (isValidDateString(formData.date) && isToday(formData.date) && isTimeBeforeNow(formData.time)) {
      newErrors.time = "O horario nao pode ser anterior ao horario atual.";
    }

    if (!formData.description.trim()) {
      newErrors.description = "Descreva o servico que precisa ser realizado.";
    }
  ...
  if (professionalId && !professional) {
    return (
      <section className="service-request-page">
        ...
        <p className="service-request-page__empty">Profissional nao encontrado.</p>
      </section>
    );
  }
    ...
    return (
      <section className="service-request-page">
        ...
        <p className="service-request-page__empty">
          {professionalId ? "Servico nao encontrado para este profissional." : "Servico nao encontrado."}
        </p>
      </section>
    );
  }
