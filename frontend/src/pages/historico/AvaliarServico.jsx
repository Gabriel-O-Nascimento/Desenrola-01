...
export default function AvaliarServico() {
  ...
  useEffect(() => {
    api.getCompletedServiceById(id)
      ...
      .catch((error) => {
        console.error("Erro ao carregar servico para avaliacao:", error);
        setService(null);
      });
  }, [id]);
  ...
  async function handleSubmit(event) {
    ...
    if (!rating) {
      setRatingError("Selecione uma nota para sua experiencia.");
      return;
    }
  ...
  if (!service) {
    return (
      <section className="review-service-page">
        ...
        <p className="review-service-empty">Servico para avaliacao nao encontrado.</p>
      </section>
    );
  }
  ...
  return (
    <section className="review-service-page">
      ...
      <form className="review-service-content" onSubmit={handleSubmit}>
        ...
        <section className="review-service-card">
          <h2 className="review-service-card__title">Como foi sua experiencia?</h2>
